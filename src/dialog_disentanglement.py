...
class PyTorchModel(nn.Module):
    ...
    def forward(self, features, word_ids=None):
        if self.embeddings:
            # Word_ids are provided as a list of lists for each option
            # Get word embeddings for each option
            word_embeddings = [self.embeddings(torch.tensor(ids)) for ids in word_ids]
            # Compute maximum and mean of word embeddings for query and option
            qvec_max = torch.max(word_embeddings[0], dim=0)[0]
            qvec_mean = torch.mean(word_embeddings[0], dim=0)
            ovec_max = torch.max(word_embeddings[1], dim=0)[0]
            ovec_mean = torch.mean(word_embeddings[1], dim=0)
            # Concatenate them with features
            features = torch.cat([features, qvec_max, qvec_mean, ovec_max, ovec_mean], dim=0)
        else:
